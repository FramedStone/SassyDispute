...
contract SassyJugglerDemo is AccessControl {
    /*
        SassyJugglerDemo roles setup
    */
    ...
    */
    ...
    function addDisputeCases(string[] memory _disputeCasesDemo) external onlyRole(ADMIN) {
        uint256[] memory caseIDs = new uint256[](_disputeCasesDemo.length);

        for (uint256 i = 0; i < _disputeCasesDemo.length; i++) {
            _disputeCases.push(disputeCase({
                ipfsHash: _disputeCasesDemo[i],
        ...
        emit disputeCasesAdded(caseIDs, _disputeCasesDemo, msg.sender);
    }
